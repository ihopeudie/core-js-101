...
function getPolynom(...args) {
  ...
  return (x) => coefficients.map((arg, i) => {
    if (i === 0) {
      return arg;
    }
    return arg * (x ** i);
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map((e) => JSON.stringify(e)).join()}) starts`);
    const val = func.apply(this, args);
    logFunc(`${func.name}(${args.map((e) => JSON.stringify(e)).join()}) ends`);
    return val;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return (...args) => fn.apply(this, args1.concat(args));
}
